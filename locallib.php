...
function hvp_get_core_assets($context) {
    ...
    foreach (\H5PCore::$scripts as $script) {
        $scriptpath = $relpath . $script . $cachebuster;
...
function hvp_add_editor_assets($id = null, $mformid = null) {
    ...
    $PAGE->requires->js('/mod/hvp/editor/scripts/h5peditor-editor.js' . $cachebuster, true);
    $PAGE->requires->js('/mod/hvp/editor/scripts/h5peditor-init.js' . $cachebuster, true);
    $PAGE->requires->js('/mod/hvp/editor.js' . $cachebuster, true);
    ...
    $PAGE->requires->js('/mod/hvp/' . $languagescript . $cachebuster, true);
...
function hvp_admin_add_generic_css_and_js($page, $relpath, $settings = null) {
    ...
    foreach (\H5PCore::$adminScripts as $script) {
        $page->requires->js($relpath . $script . hvp_get_cache_buster(), true);
    }
